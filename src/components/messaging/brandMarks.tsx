...
/**
 * Sign-in provider marks, inlined rather than fetched (ported from
 * juicebox.money's BrandMarks). Paths are the official marks from
 * Simple Icons (CC0), unmodified; brand colours are the official ones,
 * and the marks that ship as pure black render in the surrounding text
 * colour.
 */
...
export const OAUTH_METHODS: { method: string; label: string }[] = [
...
export function WalletFallbackMark({
  ...
  return (
    ...
    >
      ...
      <path d="M2.75 9.25h18.5" strokeLinecap="round" />
      <circle cx="17.25" cy="14" r="1.15" fill="currentColor" stroke="none" />
    </svg>
  );
}

/** The wallets to offer, in the order to offer them (juicebox.money's
 * dedupe rules: the generic injected connector only when EIP-6963
 * announced nothing; name collisions collapse and the announced entry
 * wins because only a discovered provider carries an icon). */
export function offerableWallets<
  T extends { id: string; name: string; icon?: string | undefined },
>(connectors: readonly T[]): T[] {
  const external = connectors.filter((connector) => connector.id !== "para");
  const discovered = external.filter(
