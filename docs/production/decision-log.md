...
| ENG-003 | Key Transparency log, independent witness/operator, checkpoint gossip, and outage behavior | Identity Security + Operations | Design half closed before G2 by ADR 0002 (`docs/adr/0002-witness-design.md`, 2026-08-18: RFC 6962 accumulator, jb-msg-witness-checkpoint/v1 preimage, 300-second freshness, overlap-based transfer); the independent operational witness deployed and evidenced before G3 remains open | New device enrollment and sensitive sends remain disabled until the witness service is deployed; self-operation never satisfies the G3 half |
...
| ENG-004 | Chain-specific finality thresholds, canonicality recheck cadence, RPC quorum, and exceptional pause rules — engineering half ratified by ADR 0005 (docs/adr/0005-finality-profiles.md, 2026-08-18): `finalized` tag only across all eight chains' three finality families, two-independent-provider quorum with hash agreement under the lowest reported head, 60-second recheck cadence, reorg-pauses-profile rule, and the `jbm-evm-adapter.1` digest conventions | Chain Integrations + Authorization Security | Per-chain operational sign-off remains: per testnet before its G3/G4 admission; per production chain before its G5 shadow | The affected chain cannot issue an eligibility lease or participate in shadow evidence |
