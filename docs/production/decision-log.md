...
| --- | --- | --- | --- | --- |
| ENG-001 | Exact audited MLS release, crypto provider, bindings, and build profile for the fixed v1 ciphersuite after the comparative bake-off | Protocol Security | Before G1 promotion, after Candidate B runs the common harness and comparative selection is approved | Candidate-A work, including `RC2`, remains a pre-G1 lab; native production messaging remains disabled |
